...
bool is_free(double const street_angle, rbout roundabout) {
  for (int i{0}; i < static_cast<int>(roundabout.size_rbout()); i++) {
    ...
    if ((std::abs(a_d_degree) < 20.) || (std::abs(a_d_degree) > 350.)) {
      return false;
    }
  }
  return true;
...
void car::evolve_tplus() { t_ += 0.02; }
void car::evolve_tminus() { t_ -= 0.02; }
void car::evolve_ang(double h) { theta_ += 0.02*h; }
...
void rbout::erase_rbt() {
  if (!empty_rbout()) {
    for (int i{0}; i < static_cast<int>(size_rbout()); i++) {
      ...
      if (std::abs(module360(angle_difference)) < 0.01) {
...
void rbout::evolve_rbt(double const h) {
  if (!empty_rbout()) {
    for (int i{0}; i < static_cast<int>(size_rbout()); i++) {
      double const angle_difference =
          std::abs(car_rbout[i].theta() -
                   2 * M_PI / n_roads() * (car_rbout[i].exit() - 1));
      if (!(std::abs(module360(angle_difference)) < 0.001)) {
        car_rbout[i].evolve_ang(h);
      }
    }
  }
}
...
void road::newcar_rd(bool const input) {
  if (input) {
    if ((static_cast<int>(size_in()) < 10) && (can_generate(275))) {
...
void road::evolve_rd(bool const input, rbout& roundabout) {
  if (input) {  // input == true
    for (int i = 0; i < static_cast<int>(size_in()); i++) {
      ...
      if ((i != 0) && ((car_in[i]).t() < 0.95) &&
          (std::abs((car_in[i]).t() - (car_in[i - 1]).t()) >= 0.2)) {
        car_in[i].evolve_tplus();
      }
    }
  } else {
