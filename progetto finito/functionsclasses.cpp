...
bool can_generate(int const parameter) {       
  std::uniform_int_distribution<int> unif(1, parameter);
...
bool is_free(double const street_angle, rbout roundabout) {
  for (int i{0}; i < static_cast<int>(roundabout.size_rbout()); i++) {
    double const angle_difference = std::abs(street_angle + 0.0665682 - module360(((roundabout.carrbout())[i]).theta()));
...
void car::evolve_ang() { theta_ += 0.002; }
...
double rbout::rad() { return radius_; }     //non lo usiamo mai, lo togliamo? serve solo come "unità di misura" nella interfaccia grafica
...
void rbout::erase_rbt() {
  if (!empty_rbout()) {
    for (int i{0}; i < static_cast<int>(size_rbout()); i++) {
      ...
      double const angle_difference = std::abs(car_rbout[i].theta() - 2 * M_PI / n_roads() * (car_rbout[i].exit() - 1) + 0.0665682);
      if (std::abs(module360(angle_difference)) < 0.005) {
...
void rbout::evolve_rbt() {
  if (!empty_rbout()) {
    for (int i{0}; i < static_cast<int>(size_rbout()); i++) {
      double const angle_difference = std::abs(car_rbout[i].theta() - 2 * M_PI / n_roads() * (car_rbout[i].exit() - 1));
      if (!(std::abs(module360(angle_difference)) < 0.001)) {
...
void road::newcar_rd(bool const input) {
  if (input) {
    if ((size_in() < 10) && (can_generate(275))) {
...
void road::evolve_rd(bool const input, rbout roundabout) {
  if (input) { // input == true
    for (int i = 0; i < static_cast<int>(size_in()); i++) {  
      ...
      if ((i == 0) && ((car_in[i]).t() > 0.95) && (is_free(angle(), roundabout))) {
        car_in[i].evolve_tplus();
        car_in[i].can_I_enter_Y(true);
      }

      if ((i != 0) && ((car_in[i]).t() < 0.95) && (std::abs((car_in[i]).t() - (car_in[i - 1]).t()) > 0.1)) {
        car_in[i].evolve_tplus();
      }
    }
  } else {
