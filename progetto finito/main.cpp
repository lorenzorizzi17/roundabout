...
int main() {
  ...
  if (!texture.loadFromFile( "texture.jpg", sf::IntRect(0, 0, display_height, display_height))) {
  ...
  cerchio1.setPosition(0.5 * display_height - 1.1 * radius, 0.5 * display_height - 1.1 * radius);
  cerchio2.setPosition(0.5 * display_height - 0.9 * radius, 0.5 * display_height - 0.9 * radius);
  while (window.isOpen()) {
    sf::Event event;
    ...
    for (int i = 1; i <= roundabout.n_roads(); i++) {
      ...
      sf::RectangleShape strada(sf::Vector2f(radius, 0.2 * radius));
      strada.setOrigin(0.5 * radius, 0.5 * 0.2 * radius);
      strada.setTexture(&texture);
      strada.setPosition( 0.5 * display_height + (std::cos(sfasorad)) * (radius * 1.5), 
                          0.5 * display_height - (std::sin(sfasorad)) * (radius * 1.5));
    ...
    for (auto it = roads.begin(); it != roads.end(); ++it) {
      ...
      for (car& c : (*it).carin()) {
        ...
        pallino.setPosition(0.5 * display_height +
                                ...
                                radius * std::cos(M_PI - (*it).angle()) +
                                c.t() * radius * std::cos(M_PI - (*it).angle()),
                            0.5 * display_height -
                                radius * std::sin((*it).angle() + 0.0665682) -
                                radius * std::sin(M_PI - (*it).angle()) +
                                c.t() * radius * std::sin(M_PI - (*it).angle()));
        window.draw(pallino);
      }
      for (car& c_ : (*it).carout()) {
        ...
        pallino_.setPosition(0.5 * display_height +
                                 radius * std::cos((*it).angle() - 0.0665682) -
                                 radius * std::cos(M_PI - (*it).angle()) +
                                 c_.t() * radius * std::cos(M_PI - (*it).angle()),
                             0.5 * display_height -
                                 radius * std::sin((*it).angle() - 0.0665682) -
                                 radius * std::sin(M_PI - (*it).angle()) +
                                 c_.t() * radius * std::sin(M_PI - (*it).angle()));
      ...
      for (car& d : roundabout.carrbout()) {
        ...
        pallino__.setPosition(0.5 * display_height + radius * std::cos(d.theta()),
