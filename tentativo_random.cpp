...
/*int random_call(int n, int roadno) {   //per adesso così va
  //int a = std::rand() % (n) + 1;
  std::uniform_int_distribution<int> rnd_ex(1, n);
  int a = rnd_ex(eng);
    while (a == roadno) {
      a = std::rand() % (n) + 1;
    }
...
int random_exit() { //da modificare i parametri di random call
  std::uniform_int_distribution<int> rnd_ex(1, n);
  int a = rnd_ex(eng);
  return a;
}

double random_entrance() { //da modificare ancora volendo con std::uniform_real_distribution<double>;
  std::uniform_real_distribution<double> rnd_en(0.09,0.85); //perché se venisse 0 non verrebbe spawnata nessuna macchina, mentre se venisse 1 verrebbero spawnate sempre
  double b = rnd_en(eng);
  return b;
}
...
double approx(double n) { //da mettere come funzione interna
  double a = n;
  while (a > 0) {
    a = a - 2 * M_PI;
  }
...
bool is_free(double ang, rbout rotonda) { //da implementare angolo 0/360
...
int main() {
  ...
  for (int k = 0; k < n; k++)
    {
      road strada_test(2*k*M_PI/n, random_entrance());
